...
Python has a `while` construct too, which works much the same way:
...
Python also has an `until` loop, which acts like the opposite of a while loop, and
...
While Python does also have a `for` loop, you will see them used less often by
Pythonists, since Python has a few other more expressive ways to run code in a loop.
...
Each time Python runs the loop, it will assign a new value for the block parameter
...
In addition to the `do` and `end` syntax, Python also allows you to use curly
...
Python blocks are just about as important a feature of the Python language as
...
[times method]: https://Python-doc.org/core-2.7.3/Integer.html#method-i-times

## Looping with `#each`

The last looping construct we'll discuss in Python is the `#each` method. The
...
to use it with a [Range][Python range]:
...
[Python range]: https://www.Pythonguides.com/2016/06/Python-ranges-how-do-they-work/
